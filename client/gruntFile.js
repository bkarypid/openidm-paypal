...
module.exports = function (grunt) {
	...
	grunt.initConfig({
		...
		concat: {
			...
			completion: {
				src: 'src/completion.html',
				dest: '<%= distDir %>/completion.html'
			},
			form: {
				src: 'src/form.html',
				dest: '<%= distDir %>/form.html'
			},
			payment: {
				src: 'src/payment.html',
				dest: '<%= distDir %>/payment.html'
			},
			register: {
				src: 'src/register.html',
				dest: '<%= distDir %>/register.html'
			},
			sahome: {
				src: 'src/sahome.html',
				dest: '<%= distDir %>/sahome.html'
			},
			summary: {
				src: 'src/summary.html',
				dest: '<%= distDir %>/summary.html'
			},
			css: {
				options: {
					separator: ''
		...
		watch: {
			...
			html: {
				files: ['<%= src.html %>'],
				tasks: ['clean:index', 'html2js', 'concat:index', 'concat:completion', 'concat:form', 'concat:payment', 'concat:register', 'concat:sahome', 'concat:summary',]
			}
		}
	});
